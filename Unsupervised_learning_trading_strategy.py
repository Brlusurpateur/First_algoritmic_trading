...
def optimize_weights(prices, lower_bound=0):
    ...
    #Calculate average historical yields
    ...
    #Ensure that the covariance matrix is symmetrical
    cov = (cov + cov.T) / 2
    #print("Dimensions des prix:", prices.shape)
    #print(cov)

    #Initialise the EfficientFrontier object
    ef = EfficientFrontier(expected_returns=returns,
                           cov_matrix=cov,
                           weight_bounds=(lower_bound, .1),
                           solver='SCS')
    
    #Optimising weights to maximise the sharpe ratio
...
for start_date in fixed_dates.keys():
    ...
    try:
        ...
        try:
            ...
            #print(round(1/(len(optimization_df.columns)*2),3))
        ...
        temp_df = pd.DataFrame(returns_dataframe[start_date:end_date])

        # Impression pour vérifier les colonnes avant la fusion
        print("Colonnes de temp_df avant la fusion:", temp_df.columns)
        print("Colonnes de weights avant la fusion:", weights.columns)
...
portfolio_df.plot()
# Print the fixed_dates keys to check which dates are actually present
#print("Clés de fixed_dates:", list(fixed_dates.keys()))